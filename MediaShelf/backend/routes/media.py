from flask import Blueprint, jsonify, request
from database import db
from models import Media, Category
import jwt
import requests

media_bp = Blueprint('media', __name__)

SECRET_KEY = 'mediashelf_secret_key'

def get_current_user(request):
    token = request.headers.get('Authorization', '').replace('Bearer ', '')
    if not token:
        return None
    try:
        return jwt.decode(token, SECRET_KEY, algorithms=['HS256'])
    except:
        return None

# Obtener todas las categorías
@media_bp.route('/categories', methods=['GET'])
def get_categories():
    categories = Category.query.all()
    return jsonify([{'id': c.id, 'name': c.name} for c in categories])

# Obtener todo el catálogo (con filtro opcional por categoría)
@media_bp.route('/all', methods=['GET'])
def get_all_media():
    category_id = request.args.get('category_id')
    query = Media.query
    if category_id:
        query = query.filter_by(category_id=category_id)
    media_list = query.all()
    return jsonify([{
        'id': m.id,
        'title': m.title,
        'cover_url': m.cover_url,
        'category_id': m.category_id,
        'year': m.year,
        'description': m.description
    } for m in media_list])

# Agregar título manualmente (solo admin)
@media_bp.route('/add', methods=['POST'])
def add_media():
    user = get_current_user(request)
    if not user or user.get('role') != 'admin':
        return jsonify({'error': 'No autorizado'}), 403

    data = request.get_json()
    if not data.get('title') or not data.get('category_id'):
        return jsonify({'error': 'Título y categoría son requeridos'}), 400

    new_media = Media(
        title=data['title'],
        cover_url=data.get('cover_url'),
        category_id=data['category_id'],
        year=data.get('year'),
        description=data.get('description')
    )
    db.session.add(new_media)
    db.session.commit()
    return jsonify({'message': 'Título agregado', 'id': new_media.id}), 201

# Buscar en TMDB y agregar automáticamente
@media_bp.route('/search/tmdb', methods=['GET'])
def search_tmdb():
    query = request.args.get('q')
    if not query:
        return jsonify({'error': 'Query requerido'}), 400

    TMDB_KEY = 'TU_API_KEY_AQUI'
    url = f'https://api.themoviedb.org/3/search/multi?api_key={TMDB_KEY}&query={query}&language=es-MX'
    response = requests.get(url)
    data = response.json()

    results = []
    for item in data.get('results', [])[:10]:
        results.append({
            'tmdb_id': item.get('id'),
            'title': item.get('title') or item.get('name'),
            'cover_url': f"https://image.tmdb.org/t/p/w500{item.get('poster_path')}" if item.get('poster_path') else None,
            'year': (item.get('release_date') or item.get('first_air_date') or '')[:4],
            'description': item.get('overview'),
            'type': item.get('media_type')
        })
    return jsonify(results)

# Detalle de un título
@media_bp.route('/<int:media_id>', methods=['GET'])
def get_media(media_id):
    m = Media.query.get_or_404(media_id)
    return jsonify({
        'id': m.id,
        'title': m.title,
        'cover_url': m.cover_url,
        'category_id': m.category_id,
        'year': m.year,
        'description': m.description
    })

# Eliminar título (solo admin)
@media_bp.route('/<int:media_id>', methods=['DELETE'])
def delete_media(media_id):
    user = get_current_user(request)
    if not user or user.get('role') != 'admin':
        return jsonify({'error': 'No autorizado'}), 403

    m = Media.query.get_or_404(media_id)
    db.session.delete(m)
    db.session.commit()
    return jsonify({'message': 'Título eliminado'}), 200