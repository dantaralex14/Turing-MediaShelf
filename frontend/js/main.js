...
async function searchTMDB() {
    ...
    try {
        ...
        const res = await fetch(`${API}/media/search/tmdb?q=${encodeURIComponent(query)}&type=${searchType}`)
        const data = await res.json()
...
function openSearchResultModal(item) {
    ...
    const categoryOptions = `
        <option value="1" ${tmdbType === 'tv' ? 'selected' : ''}>Series</option>
        <option value="2" ${tmdbType === 'movie' ? 'selected' : ''}>Películas</option>
